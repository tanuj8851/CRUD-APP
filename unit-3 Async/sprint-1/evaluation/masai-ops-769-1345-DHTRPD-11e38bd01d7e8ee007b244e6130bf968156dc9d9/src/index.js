...
function studentData(firstName, lastName, age, marksArray, ...rest) {
    ...
    let obj = {
        ...
        "hobbies": [...rest],
        getInfo: (fullName) => {
            let x = `${firstName} ${lastName}'s age is ${age}.`
            return x;
        },
        getResult: () => {
            let sum = 0;
...
function massageArray(inputArray) {
    ...
    let eo1 = inputArray.reduce((acc, item) => {
        ...
        obj.Ingredients.push({ ingredient: item.strIngredient1, measure: item.strMeasure1 })
        obj.Ingredients.push({ ingredient: item.strIngredient2, measure: item.strMeasure2 })
        obj.Ingredients.push({ ingredient: item.strIngredient3, measure: item.strMeasure3 })
        obj.Ingredients.push({ ingredient: item.strIngredient4, measure: item.strMeasure4 })
        obj.Ingredients.push({ ingredient: item.strIngredient5, measure: item.strMeasure5 })
        obj.Ingredients.push({ ingredient: item.strIngredient6, measure: item.strMeasure6 })
        obj.Ingredients.push({ ingredient: item.strIngredient7, measure: item.strMeasure7 })
        obj.Ingredients.push({ ingredient: item.strIngredient8, measure: item.strMeasure8 })
        obj.Ingredients.push({ ingredient: item.strIngredient9, measure: item.strMeasure9 })
        if (item.strIngredient10) {
            obj.Ingredients.push({ ingredient: item.strIngredient10, measure: item.strMeasure10 })
        }
